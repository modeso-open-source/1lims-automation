...
class ContactsTestCases(BaseTest):
    ...
    def test_008_create_contact_person_from_edit_update_old_value(self):
        ...
        LIMS-6388
        """
        self.contacts_page.info('open random contact record to add a new contact persons to it')
        ...
        self.contacts_page.info('Open contact persons page')
        self.contact_page.get_contact_persons_page()
        self.contacts_page.info('add new record to contact persons')
        contact_persons_after_update = self.contact_page.create_update_contact_person(save=True)

        self.contacts_page.info('Refresh to compare the data before and after refresh')
        self.base_selenium.refresh()
