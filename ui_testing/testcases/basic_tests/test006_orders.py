...
class OrdersTestCases(BaseTest):
    ...
    def test107_check_multiple_contact_in_export_file(self):
        """
        ...
        row = self.orders_page.result_table()[0]
        self.orders_page.click_check_box(row)
        self.info('download excel sheet of order {}'.format(payload[0]['orderNo']))
        self.order_page.download_xslx_sheet()
        contacts_in_sheet = self.order_page.sheet.iloc[0]['Contact Name'].split(', ')
        self.assertCountEqual(contacts_in_sheet, contacts)