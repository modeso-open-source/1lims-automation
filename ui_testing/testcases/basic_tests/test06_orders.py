...
class OrdersTestCases(BaseTest):
    ...
    def test003_cancel_button_edit_departments(self, save):
        ...
        if 'save_btn' == save:
            ...
            self.info(
                ' + Assert {} (current_department) == {} (order_departments)'.format(current_department,order_department))
    ...
    def test011_duplicate_many_orders(self):
        ...
        self.info('get last order row that you created')
        orders, payload = self.orders_api.get_all_orders(limit=20)
        order = random.choice(orders['orders'])
        self.info(
            '{}'.format(order['orderNo']))
        ...
        data_before_duplicate_sub_order = self.base_selenium.get_row_cells_dict_related_to_header(
                                                                row=child_rows[0], table_element='general:table_child')
        ...
        for index in range(no_of_copies):
            ...
            self.info('Check if order created number:  {} has Test unit = {}   '.format(index + 1,
    ...
    def test012_update_suborder_materialtype(self):
        ...
        article.
        New: Orders: Materiel type Approach: In case then material type of the second suborder updated then press on cancel button,
        ...
        self.info(
            " Duplicate it to make sure that I have two subordes to update the second one ")
    ...
    @skip("https://modeso.atlassian.net/browse/LIMSA-217")
    ...
    def test027_create_existing_order_with_test_units_and_change_material_type(self):
        """
