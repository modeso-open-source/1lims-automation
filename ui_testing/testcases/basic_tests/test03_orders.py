...
class OrdersTestCases(BaseTest):
    ...
    def test018_validate_order_no_exists(self):
        ...
        """
        self.base_selenium.LOGGER.info(
            ' Running test case to check that order number should be unique')
        
        order_no_created =  self.order_page.create_new_order(material_type='r', article='a', contact='a', test_plan='r',
                                         test_unit='r', multiple_suborders=0)
        ...
        self.order_page.set_no(no=order_no_created)
        time.sleep(self.base_selenium.TIME_MEDIUM)
