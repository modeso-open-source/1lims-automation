...
class TestUnitsTestCases(BaseTest):
    ...
    def test_api_article(self):
        random_article_name=self.test_unit_page.generate_random_text()
        random_article_number=self.test_unit_page.generate_random_number()
        # material_type=self.general_utilities_api.list_all_material_types()[0]
        material_type = {
            'text': self.test_unit_page.generate_random_text(),
            'id': 'new'
        }
        self.base_selenium.LOGGER.info(self.article_api.create_article(name=random_article_name, No=random_article_number, materialType=material_type, selectedMaterialType=[material_type], materialTypeId=material_type['id']))
    
    def test_api_testunit(self):
        random_testunit_name=self.test_unit_page.generate_random_text()
        random_testunit_no=self.test_unit_page.generate_random_number()
        material_type=self.general_utilities_api.list_all_material_types()[0]
        category = {
            'text': self.test_unit_page.generate_random_text(),
            'id': 'new'
        }
        concentrations = self.test_unit_api.list_testunits_concentrations()[1]
        self.base_selenium.LOGGER.info(self.test_unit_api.create_quantitative_testunit(name=random_testunit_name, number=random_testunit_no, category=category, selectedMaterialTypes=[material_type], useSpec=True, upperLimit=22, lowerLimit=11))
        self.base_selenium.LOGGER.info(self.test_unit_api.create_qualitative_testunit(name=random_testunit_name, number=random_testunit_no+2, category=category, selectedMaterialTypes=[material_type], textValue='mazen1,mazen2,mazen3'))
        self.base_selenium.LOGGER.info(self.test_unit_api.create_mibi_testunit(name=random_testunit_name, number=random_testunit_no+4, category=category, selectedMaterialTypes=[material_type], upperLimit=321, selectedConcs=[concentrations]))

    def test_api_role(self):
        random_role_name = self.test_unit_page.generate_random_text()
        permissons_list = self.general_utilities_api.list_all_permissions()
        ...
        self.base_selenium.LOGGER.info(self.roles_api.create_role(role_name=random_role_name, permissions=permissions))
